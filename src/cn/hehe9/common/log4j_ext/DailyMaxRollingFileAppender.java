...
/**
 * 源码地址:<p>
 * <b><i>http://wiki.apache.org/logging-log4j/DailyRollingFileAppender</i></b><p>
 * 说明: 对源码作了小部分修改, 可删除不在指定日期范围内的日志文件( 详见 rollOver() 方法 ).<p>
 * 
...
public class DailyMaxRollingFileAppender extends FileAppender {
	...
	// sequence.
	public static final int TOP_OF_TROUBLE = RollingCalendar.TOP_OF_TROUBLE;
	public static final int TOP_OF_MINUTE = RollingCalendar.TOP_OF_MINUTE;
	public static final int TOP_OF_HOUR = RollingCalendar.TOP_OF_HOUR;
	public static final int HALF_DAY = RollingCalendar.HALF_DAY;
	public static final int TOP_OF_DAY = RollingCalendar.TOP_OF_DAY;
	public static final int TOP_OF_WEEK = RollingCalendar.TOP_OF_WEEK;
	public static final int TOP_OF_MONTH = RollingCalendar.TOP_OF_MONTH;
	...
	/**
	   Do not delete backup files by default.
	 */
	...
	void rollOver() throws IOException {
		...
		if (result) {
			...
			if (maxBackupIndex > 0) {
				...
				// 根据 MaxBackupIndex, 计算日期范围
				List<String> maxBackupIndexDates = getMaxBackupIndexDates();
				// 遍历目录,将日期不在备份范围内的日志删掉  
				for (File logFile : folder.listFiles()) {
					if (logFile.getName().startsWith(file.getName()) && !logFile.getName().equals(file.getName())) {
						//获取文件名带的日期时间戳  
						String markedDate = logFile.getName().substring(file.getName().length());
						if (!maxBackupIndexDates.contains(markedDate)) {
							result = logFile.delete();
						}
						if (result) {
							LogLog.debug(logFile.getName() + " ->deleted ");
						} else {
							LogLog.error("Failed to deleted old file :" + logFile.getName());
						}
					}
				}
			}
